...
df["total_deaths"] = df.groupby(["country_name", "date"])[
    "new_deceased"
].transform("sum")
    ...
    children=[
        html.Div(
                [
                    html.Div(
                    [
                        html.H3("Effect of Govt. Responses", style={"color": "white"}),  # Customize text color
                    ],
                    style={
                ...
                dcc.Markdown("Governmental response of no restriction or recommendation of a policy is considered as no restriction. Partial or full restrictions are considered as restrictions."),
                ]
        ),
        html.Div(
            [
        ...
        html.Div(
            [
                dcc.Graph(id='government-graph', style=dict(height="76vh", overflow='scroll')),
            ],
...
def update_charts(selected_location):
    ...
    governement_response_columns = ['school_closing', 'workplace_closing', 'cancel_public_events', 
        'restrictions_on_gatherings', 'public_transport_closing', 'stay_at_home_requirements', 
        'restrictions_on_internal_movement', 'international_travel_controls', 'debt_relief', 
        'public_information_campaigns', 'testing_policy', 'contact_tracing', 'facial_coverings', 
        'vaccination_policy'
    ]
    ...
    trace1 = go.Scatter(
        ...
        hoverinfo="text",
        text=["Date: {}<br>Active Cases: {}".format(date, int(active)) for date, active in zip(dates, active_cases)],
    )
    ...
    trace2 = go.Scatter(
        ...
        line=dict(color="red"),
        hoverinfo="text",
        text=["Date: {}<br>Deaths: {}".format(date, int(death)) for date, death in zip(dates, deaths)],
    )
    ...
    trace3 = go.Scatter(
        ...
        hoverinfo="text",
        text=["Date: {}<br>Recoveries: {}".format(date, int(recovery)) for date, recovery in zip(dates, recoveries)],
    )
    ...
    trace4 = go.Scatter(
        ...
        hoverinfo="text",
        text=["Date: {}<br>Vaccinated: {}".format(date, int(vaccinated_value)) for date, vaccinated_value in zip(dates, vaccinated)],
    )

    data_line = [trace1, trace2, trace3, trace4]
    tick_labels = [date.strftime('%Y-%m-%d') if i % 10 == 0 else '' for i, date in enumerate(selected_df['date'])]


    for column in governement_response_columns:
        events = selected_df[column].tolist()
        indexes = find_transitions(events)
        y_axis_range = [min(active_cases + deaths + recoveries + vaccinated), max(active_cases + deaths + recoveries + vaccinated)]
        y_values = [y_axis_range[0], y_axis_range[1]]
        for idx in indexes:
            if events[idx] == "Restrictions":
                data_line.append(go.Scatter(x=[str(selected_df.iloc[idx]["date"])] * 2, y=y_values, yaxis='y2', mode='lines', text=f'{" ".join(word.capitalize() for word in column.split("_"))} Restriction IMPOSED', hoverinfo='text', line_dash="solid", line_color="black", showlegend=False))
            else:
                data_line.append(go.Scatter(x=[str(selected_df.iloc[idx]["date"])] * 2, y=y_values, yaxis='y2', mode='lines', text=f'{" ".join(word.capitalize() for word in column.split("_"))} Restriction REMOVED', hoverinfo='text', line_dash="dash", line_color="black", showlegend=False))
        
        layout = go.Layout(
        xaxis=dict(title="Date", tickvals=dates, ticktext=tick_labels, tickmode='array',),
