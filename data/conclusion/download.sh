wget -O demographics.csv https://storage.googleapis.com/covid19-open-data/v3/demographics.csv

wget -O epidemiology.csv https://storage.googleapis.com/covid19-open-data/v3/epidemiology.csv

wget -O vaccinations.csv https://storage.googleapis.com/covid19-open-data/v3/vaccinations.csv

wget -O economy.csv https://storage.googleapis.com/covid19-open-data/v3/economy.csv

wget -O index.csv https://storage.googleapis.com/covid19-open-data/v3/index.csv

wget -O healh.csv https://storage.googleapis.com/covid19-open-data/v3/health.csv

wget -O mobility.csv https://storage.googleapis.com/covid19-open-data/v3/mobility.csv